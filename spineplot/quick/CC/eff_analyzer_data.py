...
def main(args):
    ...
    # Input file
    rf = uproot.open('/exp/icarus/app/users/lkashur/SPINEAna/spine_anaplot_tools/cafana/pi0ana_bnbcv_onbeam_argmax_14_march_2025.root')
    pur_tree = rf['events/mc/Purity_TradCuts']
    #pur_tree = rf['events/onbeam/Purity_TradCuts']
    pur_df = pur_tree.arrays(library="pd")

    # Calculate "purity" as a function of each cut...
    cuts = {'Flash Cut':['flash_cut == 1'], 
            "Fiducial Cut":['flash_cut == 1','fiducial_cut == 1'],
            "One Muon Cut":['flash_cut == 1','fiducial_cut == 1', 'one_muon_cut == 1'],
            "No Charged Pions Cut":['flash_cut == 1','fiducial_cut == 1', 'one_muon_cut == 1', 'zero_charged_pions_cut'],
            "Two or Three Photons Cut":['flash_cut == 1','fiducial_cut == 1', 'one_muon_cut == 1', 'zero_charged_pions_cut', 'two_photons_cut'],
            #"Topology Cut":['flash_cut == 1','fiducial_cut == 1','topology_cut == 1'],
            "pi0 Mass Cut":['flash_cut == 1','fiducial_cut == 1','topology_cut == 1','pi0_mass_cut == 1']}
    
    count_events_by_cut(pur_df, cuts)

def add_plot_labels(ax,pot, adj_y=0.045, title=str()):
    yrange = ax.get_ylim()
...
def count_events_by_cut(pur_df, cuts):
    no_cut_count = len(pur_df)
    no_cut_frac = no_cut_count / len(pur_df)
    print(f'No Cut Count: {no_cut_count} ({round(100*no_cut_frac,2)}%)')
    for key, value in cuts.items():
        ...
        cut_count = len(sel_by_cut)
        cut_frac = cut_count / len(pur_df)
        print(f'{key} Count: {cut_count} ({round(100*cut_frac,2)}%)')
