...
def add_plot_labels(ax, pot, adj_y=0.025, title=str()):
    ...
    usex = xrange[0] + 0.05*(xrange[1] - xrange[0])
...
def plot_histogram(cfg, plot_cfg, var, df_mc=None, pot_mc=0, livetime_mc=0, df_onbeam=None, pot_onbeam=0, livetime_onbeam=0, df_offbeam=None, livetime_offbeam=0):
    ...
    if plot_cfg['include_mc'] == True and plot_cfg['include_mc_err'] == True and plot_cfg['include_onbeam'] == False:
        h_mc, l_mc = h[:-1], l[:-1]
        if plot_cfg['normalization'] in [None, 'data']:
            l_mc = [f'{l} ({np.sum(contents_mc[::-1][li]):.0f}, {np.sum(contents_mc[::-1][li]) / np.sum(contents_mc):.02%})'for li, l in enumerate(l_mc)]
        elif plot_cfg['normalization'] == 'absolute':
            l_mc = [f'{l} ({np.sum(contents_mc[::-1][li]) / np.sum(contents_mc):.02%})' for li,l in enumerate(l_mc)]
        h_mc = h_mc[::-1]
    ...
    if plot_cfg['fit_mc'] == True:
        ...
        popt_mc, pcov_mc = curve_fit(crystalball, centers_mc[0][:-10], bins_mc[:-10], p0=(1200, 1, 3, 135, 50, -0.05, 15), maxfev=100000)
    ...
    if plot_cfg['fit_onbeam'] == True:
        xspace=np.linspace(cfg['variables'][var]['bins'][1], cfg['variables'][var]['bins'][2], 1000)
        popt_onbeam, pcov_onbeam = curve_fit(crystalball, centers_onbeam[:-10], bins_onbeam[:-10], p0=(135, 1, 3, 135, 10, -0.01, 0.01), maxfev=100000)
...
± {round(np.sqrt(np.diag(pcov_onbeam))[4],1)} [MeV/c$^{2}$]'
    ...
    if plot_cfg['fit_mc'] == True or plot_cfg['fit_onbeam'] == True:
        l1 = ax.legend(fits, fit_labels, fontsize=14, loc='upper right', bbox_to_anchor=(1, 0.45), frameon=False) # bbox_to_anchor=(1, 0.58)
        plt.gca().add_artist(l1)

    ax.legend(h,l, ncols=1)
    if plot_cfg['normalization'] in ['absolute', 'data']:
    ...
    if plot_cfg['normalization'] == 'absolute': ax.set_ylabel('Normalized Candidates')
...
def main(args):
    ...
    #sel_nu_mc_tree = rf['events/var01/SelectedNu_TradCuts']
    sel_nu_mc_df = sel_nu_mc_tree.arrays(library='pd')
    sel_cos_mc_tree = rf['events/mc/SelectedCos_TradCuts']
    #sel_cos_mc_tree = rf['events/var01/SelectedCos_TradCuts']
