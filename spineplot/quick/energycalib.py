...
def main(args):
    ...
    #plot_muon_energy_bias(muon_df, pot, 'true_ke', 'reco_mcs_ke')
...
def plot_muon_energy_bias(muon_df, pot, x='reco_csda_ke', y='reco_calo_ke'):
    ...
    h = ax.hist2d(muon_df[x], muon_df[y], bins=np.linspace(0,2000, 51), cmap='viridis', norm=colors.LogNorm()) # icarus
    ax.plot([0, 2000], [0, 2000], color='red', linestyle='--', lw=1.5) # icarus

    #h = ax.hist2d(muon_df[x], muon_df[y], bins=np.linspace(0,1500, 101), cmap='viridis', norm=colors.LogNorm())
    #ax.plot([0, 1500], [0, 1500], color='red', linestyle='--', lw=1.5)
    
    ax.set_xlabel('True Muon Kinetic Energy [MeV]')
    ax.set_ylabel('Reco Muon MCS Kinetic Energy [MeV]')
...
def plot_shower_energy_bias(photon_df, pot):
    fig, ax = plt.subplots(figsize=(10,7))
    h = ax.hist2d(photon_df.true_photon_ke, photon_df.reco_calo_ke, bins=np.linspace(0,1000, 51), cmap='viridis', norm=colors.LogNorm()) # icarus
    ax.plot([0, 1000], [0, 1000], color='red', linestyle='--', lw=1.5) # icarus

    #h = ax.hist2d(photon_df.true_photon_ke, photon_df.reco_calo_ke, bins=np.linspace(0,800, 41), cmap='viridis', norm=colors.LogNorm())
    #ax.plot([0, 800], [0, 800], color='red', linestyle='--', lw=1.5)
