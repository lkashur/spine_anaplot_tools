...
def main(args):
    ...
    tree = rf['events/mc/SelectedNu_PhaseCuts']
    df = tree.arrays(library="pd")

    # Plot
    #plot_smearing(df, 'muon_beam_costheta', pd.read_csv('muon_beam_costheta_bins.txt'), [1.74e20,1.94e19])
    plot_smearing(df, 'muon_momentum_mag', pd.read_csv('muon_momentum_mag_bins.txt'), [1.32e21,2.0e20])
...
def plot_smearing(df, var, binning_df, scaling):

    # Only signal interactions
    df = df[df.category == 0]

    # Get true and reco variables
    truth_var = 'true_' + var
    reco_var = 'reco_' + var

    # Apply binning
    binning_df = pd.DataFrame(binning_df['bin'].to_list() + [np.inf], columns=['bin']) # prepare overflow
    #print(binning_df)
    df[f'{truth_var}_bin'] = pd.cut(df[truth_var], binning_df['bin'], labels=False)
    df[f'{reco_var}_bin'] = pd.cut(df[reco_var], binning_df['bin'], labels=False)

    label_keys = np.unique(df[f'{truth_var}_bin'])
    label_values = np.unique(df[f'{reco_var}_bin'])

    #print(df[df.isna().any(axis=1)][[reco_var, truth_var, f'{reco_var}_bin', f'{truth_var}_bin']])

    # Plot
    cm = confusion_matrix(np.array(df[f'{truth_var}_bin']), np.array(df[f'{reco_var}_bin']), normalize='true')
    cm_counts = confusion_matrix(np.array(df[f'{truth_var}_bin']), np.array(df[f'{reco_var}_bin']))
    #cm_counts = np.around((scaling[1]/scaling[0]) * cm_counts, 1)
    cm_counts = np.int_((scaling[1]/scaling[0]) * cm_counts)
