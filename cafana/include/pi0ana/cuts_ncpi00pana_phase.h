  ...
  //}
    ...
    template<class T>
	    ...
	    reco_inter_phase s = utilities_ncpi0ana_phase::reco_interaction_info(obj);
            return (utilities_ncpi0ana_phase::reco_shower_criteria(obj) && s.pi0_momentum_mag >= MIN_PI0_MOMENTUM && c[2] == 1 && c[3] == 0);
        }
    ...
    template<class T>
        ...
        {
	    std::vector<uint32_t> c(utilities_ncpi0ana_phase::count_primaries(obj));
	    return c[2] == 0 && c[3] == 0 && c[0] >= 2 && c[0] < 4 && c[4] == 0;
	}
    ...
    bool signal_0mu0pi1pi0(const caf::SRInteractionTruthDLPProxy & obj)
	  ...
	  return s.num_primary_muons_thresh == 0 && s.num_primary_pions_thresh == 0 && s.num_primary_pi0s_thresh == 1 && s.num_primary_protons_thresh == 0 && !s.is_cc && s.is_neutrino;
        }
