 ...
 */
#ifndef VARS_PI0ANA_NC_H
#define VARS_PI0ANA_NC_H
...
namespace vars::pi0ana_nc
{
    ...
    double category(const caf::SRInteractionTruthDLPProxy & obj)
    {
      ...
      // Signal   
      if(cuts::pi0ana_nc::signal_0mu1pi0(obj))
      {
	if(cuts::fiducial_cut(obj) && cuts::track_containment_cut(obj))
	{
      ...
      // Neutrino Background              
      else if(cuts::pi0ana_nc::other_nu_0mu1pi0(obj))
      {
	cat = 2;
      }
      return cat;
    ...
    double is_signal(const caf::SRInteractionTruthDLPProxy & obj)
    {
      ...
      if(s.is_neutrino)
        {
          // Signal
          if(s.num_primary_muons_thresh == 0 && s.num_primary_pi0s_thresh == 1 && !s.is_cc && s.is_fiducial && s.has_contained_tracks) cat = 1;

          // Signal (OOPS)
          else if( (s.num_primary_muons == 0 && s.num_primary_pi0s == 1 && !s.is_cc) && (s.num_primary_muons_thresh != 0 || s.num_primary_pi0s_thresh != 1 || !s.is_fiducial || !s.has_contained_tracks) ) cat = 2;
	...
	{
	  // 0mu1pi0 (in-phase)
	  if(s.num_primary_muons_thresh == 0 && s.num_primary_pi0s_thresh == 1 && !s.is_cc && s.is_fiducial && s.has_contained_tracks) cat = 0;
	  // 0mu1pi0 (OOPS)
	  else if( (s.num_primary_muons == 0 && s.num_primary_pi0s == 1 && !s.is_cc) && (s.num_primary_muons_thresh != 0 || s.num_primary_pi0s_thresh != 1 || !s.is_fiducial || !s.has_contained_tracks) ) cat = 1;
	  // 0muNpi0 
	  else if(s.num_primary_muons == 0 && s.num_primary_pi0s > 1 && !s.is_cc) cat = 2;
	  // CC 1pi0
