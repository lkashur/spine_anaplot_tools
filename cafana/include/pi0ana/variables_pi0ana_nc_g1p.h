 ...
 */
#ifndef VARS_PI0ANA_NC_G1P_H
#define VARS_PI0ANA_NC_G1P_H
...
namespace vars::pi0ana_nc_g1p
{
    ...
    double category(const caf::SRInteractionTruthDLPProxy & obj)
    {
      ...
      // Signal   
      if(cuts::pi0ana_nc_g1p::signal_0mu0pi1pi0(obj))
      {
	if(cuts::fiducial_cut(obj) && cuts::track_containment_cut(obj))
	{
      ...
      // Neutrino Background              
      else if(cuts::pi0ana_nc_g1p::other_nu_0mu0pi1pi0(obj))
      {
	cat = 2;
      }
      return cat;
    ...
    double category_topology(const caf::SRInteractionTruthDLPProxy & obj)
    {
      ...
      if(s.is_neutrino)
        {
          // 0mu1pi0 (in-phase)
          if(s.num_primary_muons_thresh == 0 && s.num_primary_pions_thresh == 0 && s.num_primary_pi0s_thresh == 1 && s.num_primary_protons_thresh >= 1 && !s.is_cc && s.is_fiducial && s.has_contained_tracks) cat = 0;
          // 0mu1pi0 (OOPS)
          else if( (s.num_primary_muons == 0 && s.num_primary_pions == 0 && s.num_primary_pi0s == 1 && s.num_primary_protons >= 1 && !s.is_cc) && (s.num_primary_muons_thresh != 0 || s.num_primary_pions_thresh != 0 || s.num_primary_pi0s_thresh != 1 || s.num_primary_protons_thresh < 1 || !s.is_fiducial || !s.has_contained_tracks) ) cat = 1;
          // 0muNpi0                                    
